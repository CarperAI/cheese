...
class WebDatasetPipeline(DatasetPipeline):
    """
    Base class for any pipeline reading from a dataset that uses webdataset format.
    Writes results to Datasets dataset

    :param url: URL to open as WebDataset
    :type url: str
    ...
    """
    def __init__(self, url : str, write_path : str, force_new : bool = False, max_length = np.inf):
        super().__init__()

        self.data_source = wds.WebDataset(url).shuffle(1000)
        self.data_source = self.preprocess_data_source(self.data_source)
        self.data_source = iter(self.data_source)
    ...
    @abstractmethod
    def preprocess_data_source(self, data_source) -> wds.WebDataset:
        """
        Further process webdataset dataset to a form that can be handled by fetch.
        """
        pass
